...
class Transcript():
    ...
    def _ctm_to_groupdata(self, data_path, group_name):
        # load stt results
    ...
    def _prepare_calculate_rms(self, wav_path, rate):
        y, _ = librosa.load(wav_path, sr=rate)
        rms_sample = librosa.feature.rms(y=y)
        y_max, y_min = np.amax(rms_sample), np.amin(rms_sample)
        return y, y_max, y_min 

    def get_transcript_rms(self, rate=8000):
        origin_ctm = self._ctm_to_groupdata(self.ctm_path, self.wav_path_origin)
        ...
        for i, ss_wav_path in enumerate(self.ss_wav_paths):
            ...
            y, y_max, y_min = self._prepare_calculate_rms(ss_wav_path, rate)
            transcript_para[i]['y'] = y
            transcript_para[i]['max'] = y_max
            transcript_para[i]['min'] = y_min

        last_row = origin_ctm.shape[0] - 1
        # 產出行員與顧客之個人逐字稿
        for i in origin_ctm.index:
            start = float(origin_ctm.loc[i, 'start_time'])
            end = round(start + float(origin_ctm.loc[i, 'stay_time']), 2) 
            ...
            for k, v in transcript_para.items():
                ...
                rms_mean_lst.append(np.mean(rms))

            max_rms = max(rms_mean_lst)
            rms_state_lst = [1 if rms == max_rms else 0 for rms in rms_mean_lst]                      

            for j, rms_state in enumerate(rms_state_lst):
                if rms_state == 1:
                    transcript_para[j]['seg'] += [text]
                    transcript_para[j]['time'] += [start, end]                
                if (rms_state != 1 or i == last_row) and transcript_para[j]['seg']:
                    transcript_para[j]['sentences'].append(' '.join(transcript_para[j]['seg']))
        ...
        print(transcript_para)
        df = pd.DataFrame(columns=['speaker', 'start', 'end', 'sentence'])
        for i, (index, _) in enumerate(times):
            ctm_amount = len(transcript_para[index]['starts'])
            if ctm_amount > 0:
                df = df.append(pd.DataFrame({'speaker': [i] * ctm_amount, 
                        'start': transcript_para[index]['starts'], 
                        'end': transcript_para[index]['ends'], 
                        'sentence': transcript_para[index]['sentences']}), ignore_index=True)
        return df
